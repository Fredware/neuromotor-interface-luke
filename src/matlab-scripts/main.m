...
clear lkmex
lkmex('start')
disp("Turn on the LUKE arm now.")
luke_connected = false;
%% LUKE Setup
motor_positions = zeros(12,1);
motor_positions(12) = -0.2;
motor_positions(6) = 0.4;
[~, luke_sensors_baseline] = updateDEKA(motor_positions);
disp('LUKE Connection Confirmed')
luke_connected = true;
...
while (ishandle(main_fig.handle))
    ...
    if ~isempty(emg_samples)
        ...
        if luke_connected
            % Send control value and read LUKE sensors
            motor_positions(1:5) = control_value;
            [~, luke_sensors] = updateDEKA(motor_positions);
        end
    end
    i_loop = i_loop + 1;
    t_loop(i_loop) = toc;
